...
class SentinelServer(tornado.websocket.WebSocketHandler):
    ...
    def send_frame(self):
        self.color, self.depth = self.realsense_stream.next()
        encoded_frame = pickle.dumps(self.color, 2)
    ...
    def on_message(self, message):
        if SentinelServer.callback != None:
            message.update({'depth': self.depth})
            SentinelServer.callback(deepcopy(pickle.loads(message)))
        else:
...
def initialize(on_new_message):
    with Streamer() as stream:
        try:
            SentinelServer.stream = stream
            SentinelServer.callback = on_new_message
