...
const int MATCHES_PER_SET = 2000;
    ...
    while(TRUE)
    {
        ...
        while(Current_Data_Set < DATA_SET_COUNT)
        {
            while(Current_Match_In_Set < MATCHES_PER_SET)
            {
                ...
                thread thread1 (Run_Match, 0);
                thread thread2 (Run_Match, 1);
                thread thread3 (Run_Match, 2);
                thread thread4 (Run_Match, 3);
                thread1.join();
                thread2.join();
                thread3.join();
                thread4.join();
                ...
                Matches[1].Current_Frame = 0;
                Matches[2].Current_Frame = 0;
                Matches[3].Current_Frame = 0;
                Init(0);
                Init(1);
                Init(2);
                Init(3);
...
void DrawScreen(HWND hWnd)
{
    ...
    for(int index = 0; index < GROUND_ROBOT_NUMBER; index++)
    {
        ...
        if(Matches[0].Array_Of_Frames[Matches[0].Current_Frame].Ground_Robots[index].Color == RED_ID)
        {
            Arena_Drawer.DrawLine(&Red_Robot_Pen, Temp_X, Temp_Y,
                Temp_X + sin(Matches[0].Array_Of_Frames[Matches[0].Current_Frame].Ground_Robots[index].Rotation * TO_RADIANS) * DIRECTION_POINTER_LENGTH * METER_PIXEL_DIST,
                Temp_Y + cos(Matches[0].Array_Of_Frames[Matches[0].Current_Frame].Ground_Robots[index].Rotation * TO_RADIANS) * DIRECTION_POINTER_LENGTH * METER_PIXEL_DIST);
        }
        else
        {
    ...
    for(int index = 0; index < OBSTACLE_ROBOT_NUMBER; index++)
    {
        ...
        Arena_Drawer.DrawLine(&Obstacle_Robot_Pen, Temp_X, Temp_Y,
            Temp_X + sin(Matches[0].Array_Of_Frames[Matches[0].Current_Frame].Obstacle_Robots[index].Rotation * TO_RADIANS) * DIRECTION_POINTER_LENGTH * METER_PIXEL_DIST,
            Temp_Y + cos(Matches[0].Array_Of_Frames[Matches[0].Current_Frame].Obstacle_Robots[index].Rotation * TO_RADIANS) * DIRECTION_POINTER_LENGTH * METER_PIXEL_DIST);
    }
    ...
    for(int index1 = 0; index1 < DRONE_NUMBER; index1++)
    {
        ...
        if(Closest_Obs_Robot_Dist < min_dist)
        {
            float Rotation_To_Closest_Obs = TO_DEGREES * atan2(Matches[In_Match].Array_Of_Frames[Matches[In_Match].Current_Frame].Obstacle_Robots[Closest_Obs_Robot].Y - Current_Drone->Y,
                                   Matches[In_Match].Array_Of_Frames[Matches[In_Match].Current_Frame].Obstacle_Robots[Closest_Obs_Robot].X - Current_Drone->X);
            Current_Drone->X_Speed = Current_Drone->X_Speed + sin((Rotation_To_Closest_Obs + 180) * TO_RADIANS) * DRONE_ACCEL;
            Current_Drone->Y_Speed = Current_Drone->Y_Speed + cos((Rotation_To_Closest_Obs + 180) * TO_RADIANS) * DRONE_ACCEL;
        }
...
void Init(int In_Match)
{
    ...
    for(int index = 0; index < GROUND_ROBOT_NUMBER; index++)
    {
        ...
        Matches[In_Match].Array_Of_Frames[0].Ground_Robots[index].X = ARENA_WIDTH / 2 + sin(Temp_Rotation * TO_RADIANS) *  METER_PIXEL_DIST;
        Matches[In_Match].Array_Of_Frames[0].Ground_Robots[index].Y = ARENA_HEIGHT / 2 + cos(Temp_Rotation * TO_RADIANS) *  METER_PIXEL_DIST;
    ...
    for(int index = 0; index < OBSTACLE_ROBOT_NUMBER; index++)
    {
        ...
        Matches[In_Match].Array_Of_Frames[0].Obstacle_Robots[index].X = FIELD_X + FIELD_WIDTH / 2 + sin(Temp_Rotation * TO_RADIANS) * METER_PIXEL_DIST * OBSTACLE_ROBOT_PATH_RADIUS;
        Matches[In_Match].Array_Of_Frames[0].Obstacle_Robots[index].Y = FIELD_Y + FIELD_HEIGHT / 2 + cos(Temp_Rotation * TO_RADIANS) * METER_PIXEL_DIST * OBSTACLE_ROBOT_PATH_RADIUS;
...
void Make_New_Sets()
{
    for(int counter = 0; counter < DATA_SET_COUNT / 4; counter++)
    {
        Data_Sets[counter + DATA_SET_COUNT / 2].Dist_From_Drone_Mult = (Data_Sets[counter * 2].Dist_From_Drone_Mult + Data_Sets[counter * 2 + 1].Dist_From_Drone_Mult) / 2;
        Data_Sets[counter + DATA_SET_COUNT / 2].Dist_From_Edge_Mult = (Data_Sets[counter * 2].Dist_From_Edge_Mult + Data_Sets[counter * 2 + 1].Dist_From_Edge_Mult) / 2;
        Data_Sets[counter + DATA_SET_COUNT / 2].Green_Line_Mult = (Data_Sets[counter * 2].Green_Line_Mult + Data_Sets[counter * 2 + 1].Green_Line_Mult) / 2;
    }
...
void Gen_Random_Set(int index)
{
    ...
    Data_Sets[index].Green_Line_Mult = static_cast<float>(distribution(generator)) / RAND_MAX * Green_Line_Range + GREEN_LINE_MULT_MIN;

    float Dist_To_Closest_Edge_Range = ((DIST_FROM_EDGE_MAX - DIST_FROM_EDGE_MIN) + 1);
    Data_Sets[index].Dist_From_Edge_Mult = static_cast<float>(distribution(generator)) / RAND_MAX * Dist_To_Closest_Edge_Range + DIST_FROM_EDGE_MIN;

    float Dist_From_Drone_Range = ((DIST_FROM_DRONE_MAX - DIST_FROM_DRONE_MIN) + 1);
    Data_Sets[index].Dist_From_Drone_Mult = static_cast<float>(distribution(generator)) / RAND_MAX * Dist_From_Drone_Range + DIST_FROM_DRONE_MIN;
    return;
}
