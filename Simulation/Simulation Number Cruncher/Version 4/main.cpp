...
struct Data_Set
{
    ...
    float Half;
    float On_Right_Side_Dist;
    float On_Left_Side_Dist;
            ...
            for(int index = 0; index < GROUND_ROBOT_NUMBER; index++)
            {
                ...
                if(Current_G_Robot->Still_In == true)
                {
                        ...
                        Matches[In_Match].Array_Of_Frames[Matches[In_Match].Current_Frame].Ground_Robots[Current_Drone->Prev_Target_Robot].Still_In == false)
                    {
                        ...
                        {
                            if(Current_G_Robot->X < FIELD_X + (Data_Sets[Current_Data_Set].On_Left_Side_Dist * METER_PIXEL_DIST))
                            {
                                if((Current_Robot_Rotation > Data_Sets[Current_Data_Set].On_Left_Rot_Min && Current_Robot_Rotation < Data_Sets[Current_Data_Set].On_Left_Rot_Max)
                                    || (Current_Robot_Rotation > Data_Sets[Current_Data_Set].On_Left_Rot_Min - 360 && Current_Robot_Rotation < Data_Sets[Current_Data_Set].On_Left_Rot_Max - 360))
                                {
                                    Temp_Score = Temp_Score + Data_Sets[Current_Data_Set].Find_Direction_Mult;
                                }
                            }
                            if(Current_G_Robot->X > FIELD_X + FIELD_WIDTH - (Data_Sets[Current_Data_Set].On_Right_Side_Dist * METER_PIXEL_DIST))
                            {
                                if(Current_Robot_Rotation > Data_Sets[Current_Data_Set].On_Right_Rot_Min && Current_Robot_Rotation < Data_Sets[Current_Data_Set].On_Right_Rot_Max)
                                {
                                    Temp_Score = Temp_Score + Data_Sets[Current_Data_Set].Find_Direction_Mult;
                                }
                            }
     ...
     //   if(oho == true)
        ...
        if(sqrt(A * A + B * B) / METER_PIXEL_DIST < TOP_TOUCH_MAX_OFFSET)
        {
            ...
            if(sqrt(A * A + B * B) / METER_PIXEL_DIST < TOP_TOUCH_MAX_SPEED_DIFF)
            {
                ...
                if(Target_G_Robot->X < FIELD_X + (Data_Sets[Current_Data_Set].On_Left_Side_Dist * METER_PIXEL_DIST))
                {
                    //if(Draw_On_Screen)
                    //cout << Rotation << " < "  << Data_Sets[Current_Data_Set].On_Left_Rot_Min << " || " << Rotation << " > " << Data_Sets[Current_Data_Set].On_Left_Rot_Max << endl;
                ...
                if(Target_G_Robot->X > FIELD_X + FIELD_WIDTH - (Data_Sets[Current_Data_Set].On_Right_Side_Dist * METER_PIXEL_DIST))
                {
...
void Output_Set(int index)
{
    ...
    cout << "Data_Sets[index].On_Left_Side_Dist = " << Data_Sets[index].On_Left_Side_Dist << ";" << endl;
    cout << "Data_Sets[index].On_Right_Side_Dist = " << Data_Sets[index].On_Right_Side_Dist << ";" << endl;
...
void Make_New_Sets(int Matches_Num)
{
    ...
    for(int counter = 0; counter < round(DATA_SET_COUNT / 4); counter++)
    {
        ...
        if(change_range(generator) <= RAND_MUT_CHANCE)
        {
            float Left_Side_Range = ((LEFT_SIDE_MAX - LEFT_SIDE_MIN) + 1);
            Data_Sets[Cur_Set].On_Left_Side_Dist = static_cast<float>(distribution(generator)) / RAND_MAX * Left_Side_Range + LEFT_SIDE_MIN;
        }
        else if(change_range(generator) <= INCREMENT_MUT_CHANCE)
        {
            Data_Sets[Cur_Set].On_Left_Side_Dist += rand_change(generator);
        }

        if(change_range(generator) <= RAND_MUT_CHANCE)
        {
            float Right_Side_Range = ((RIGHT_SIDE_MAX - RIGHT_SIDE_MIN) + 1);
            Data_Sets[Cur_Set].On_Right_Side_Dist = static_cast<float>(distribution(generator)) / RAND_MAX * Right_Side_Range + LEFT_SIDE_MIN;
        }
        else if(change_range(generator) <= INCREMENT_MUT_CHANCE)
        {
            Data_Sets[Cur_Set].On_Right_Side_Dist += rand_change(generator);
        }
...
void Gen_Random_Set(int index)
{
    ...
    float Left_Side_Dist_Range = ((LEFT_SIDE_MAX - LEFT_SIDE_MIN) + 1);
    Data_Sets[index].On_Left_Side_Dist = static_cast<float>(distribution(generator)) / RAND_MAX * Left_Side_Dist_Range + LEFT_SIDE_MIN;

    float Right_Side_Dist_Range = ((RIGHT_SIDE_MAX - RIGHT_SIDE_MIN) + 1);
    Data_Sets[index].On_Right_Side_Dist = static_cast<float>(distribution(generator)) / RAND_MAX * Right_Side_Dist_Range + LEFT_SIDE_MIN;
    ...
    if(Evo_Iteration == 0)
    {
        ...
        Data_Sets[index].Dist_From_Drone_Mult = 0.000;
        Data_Sets[index].Head_Off_Edge_Mult = -5;
        ...
        Data_Sets[index].On_Left_Side_Dist = 0;
        Data_Sets[index].On_Right_Side_Dist = 0;
...
/*
    if(Evo_Iteration == 0)
    {
        ...
        Data_Sets[index].On_Left_Side_Dist = 9.24595;
        Data_Sets[index].On_Right_Side_Dist = 3.78735;
